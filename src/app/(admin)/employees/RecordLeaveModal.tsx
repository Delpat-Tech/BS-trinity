...
export default function RecordLeaveModal({ employees, trigger, open: externalOpen, onOpenChange }: { employees: any[], trigger?: React.ReactElement<any>, open?: boolean, onOpenChange?: (open: boolean) => void }) {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      await addLeave(Number(formData.get("employeeId")), {
      ...
      {open && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-[2px]">
          <div className="bg-surface rounded-lg shadow-xl w-full max-w-md p-6 border border-border">
            <h2 className="text-[18px] font-semibold mb-4">Record Leave</h2>
